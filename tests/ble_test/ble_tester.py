...
def on_notify(_handle: int, data: bytearray) -> None:
    ...
    # Unpack Channel 1 Data (First 120 bytes)
    ...
    # Unpack Channel 2 Data (Last 120 bytes)
    for i in range(0, 120, 3):
...
async def ble_task() -> None:
    ...
    if device is None:
        print("[ERROR] device not found — check DEVICE_NAME matches prj.conf")
        return
    ...
    async with BleakClient(device) as client:
        ...
        print("Subscribed. Streaming … (Press Ctrl+C to stop)\n")
        
        # This keeps the async loop alive indefinitely while waiting for notifications
        await asyncio.Future()

if __name__ == "__main__":
    try:
        # Run the async loop directly in the main thread
